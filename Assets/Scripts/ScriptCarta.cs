...
public class ScriptCarta : MonoBehaviour
{
    ...
    public void ViraCarta()
    {
        ...
        cartaFrente = controladorCarta.GetComponent<Controlador>().PegaFrenteCarta(valorCarta,false);
        cartaCostas = controladorCarta.GetComponent<Controlador>().PegaCostasCarta();
        if (!estado && !bloqueado && !resolvida)
        {
